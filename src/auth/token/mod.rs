...
mod tests {
    ...
    fn test_validate_web_token_ok() -> Result<()> {
        ...
        // -- Exec
        thread::sleep(Duration::from_millis(10)); // Simulate time passing, but token should still be valid.
        let res = validate_web_token(&fx_token, fx_salt);

        // -- Check
        res?; // Should pass validation without error.
    ...
    fn test_validate_web_token_err_expired() -> Result<()> {
        ...
        // -- Exec
        thread::sleep(Duration::from_millis(20)); // Simulate time passing until token expires.
        let res = validate_web_token(&fx_token, fx_salt);
